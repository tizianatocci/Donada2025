...
## Prepare input for single variable TF-MIIC
```{r}
...
## Write table with info_shifted for each TF and each cell
```{r}
df <- all_res
df <- do.call(cbind, df)
```
...
df <- read.csv(sprintf("filtering\\%s.csv", n_try), header=T, row.names="X")
...
## Save input
```{r}
df_family <- save_input(my_TF, stem.combined, df.merged, 
                        vars, vars2, 'wtfamily_thv1', 
                        sprintf("%s", n_try))
prova_check(df_family, dataset1, "exp1")
prova_check(df_family, dataset3, "exp3")
#without family
vars <- filter_info(df%>%select(-c("family")),th1, to_delete)
vars2 <- filter_info(df%>%select(-c("family")),th2, to_delete)
df_wo_family <- save_input(my_TF[-1], stem.combined, df.merged%>% select(-c("family")), 
                           vars, vars2, 'wofamily_thv1', 
                           sprintf("%s", n_try))
prova_check(df_wo_family, dataset1, "exp1")
prova_check(df_wo_family, dataset3, "exp3")
